...
parser.add_argument("depth",help="min size of #C+#T")
parser.add_argument("region",help="size of region")
parser.add_argument("quaified",help="min number of region member")
parser.add_argument("DMR_CG_cutoff",help="DMR_CG_cutoff")
parser.add_argument("DMR_CHH_cutoff",help="DMR_CHH_cutoff")
parser.add_argument("DMR_CHG_cutoff",help="DMR_CHG_cutoff")
parser.add_argument("heat_cutoff",help="Heatmap_cutoff")
parser.add_argument("bin_size",help="resolution of chrView and Metaplot")
parser.add_argument("promoter_size",help="promotor_size")
parser.add_argument("input_gene_name",help="path of input gene")

args = parser.parse_args()

depth=int(args.depth)
quaifiedSite=int(args.quaified)
pca_heat_cut=float(args.heat_cutoff)
dmr_cut=float(args.DMR_CG_cutoff)
promoter=int(args.promoter_size)
binSize=int(args.bin_size)
input_gene=pd.read_csv(str(args.input_gene_name),sep='\t',header=None)
input_gene_name=str(args.input_gene_name)

#Choose Tools
Heatmap_PCA=raw_input('Use Heatmap & PCA Tool?  (y/n)')
Fold_Enrichment=raw_input('Use Fold Enrichment Tool?  (y/n)')
DMG=raw_input('Use DMG Tool?  (y/n)')
ChrView=raw_input('Use Chromosome View Tool?  (y/n)')
Metaplot=raw_input('Use Metaplot Tool?  (y/n)')

if(ChrView=='y'):
    chrview_lo=str(raw_input('enter left para of chrview'))
    chrview_ro=str(raw_input('enter right para of chrview'))
else:
    pass

if(Metaplot=='y'):
    metaplot_lo=str(raw_input('enter left para of metaplot'))
    metaplot_ro=str(raw_input('enter right para of metaplot'))
    metaplot_gene_feature=str(raw_input('enter one genome feature of metaplot'))
else:
    pass
...
#combined[['chr']] = combined[['chr']].astype(int) need process,chromosome
...
CG_delta=np.append(dmr_cut,CG_data[1:,index2_CG].astype(np.float).mean(axis=1)-CG_data[1:,index1_CG].astype(np.float).mean(axis=1)) #diffrence of average

CHH_delta=np.array(0.0)
CHH_delta=np.append(dmr_cut,CHH_data[1:,index2_CHH].astype(np.float).mean(axis=1)-CHH_data[1:,index1_CHH].astype(np.float).mean(axis=1)) #diffrence of average

CHG_delta=np.array(0.0)
CHG_delta=np.append(dmr_cut,CHG_data[1:,index2_CHG].astype(np.float).mean(axis=1)-CHG_data[1:,index1_CHG].astype(np.float).mean(axis=1)) #diffrence of average
...
subprocess.call('''python ./extract_transcript_regions.py -i %s -o %s --gtf'''%(input_gene_name,input_gene_name), shell=True)
...
if(ChrView=='y'):
    ...
    pd.DataFrame([['samples difference','chrView_mean.txt']]).to_csv('chrView_mean_list.txt',sep='\t',index=False,header=None)
