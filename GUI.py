...
class Panel(qw.QWidget):
    def __init__(self):
        ...
        label7 = qw.QLabel("DMR_CG_CutOffValue")
        self.tbn4=qw.QLineEdit("0.2")
        self.tbn4.setValidator(qg.QDoubleValidator())
        label8 = qw.QLabel("DMR_CHH_CutOffValue")
        self.tbn5=qw.QLineEdit("0.2")
        self.tbn5.setValidator(qg.QDoubleValidator())
        label9 = qw.QLabel("DMR_CHG_CutOffValue")
        self.tbn6=qw.QLineEdit("0.2")
        self.tbn6.setValidator(qg.QDoubleValidator())
        labelpcacut=qw.QLabel("Heat_CutOffValue")
        self.tbnpcacut=qw.QLineEdit("0.2")
        self.tbnpcacut.setValidator(qg.QDoubleValidator())
        labelbinsize=qw.QLabel("Bin Size")
        self.tbnbinsize=qw.QLineEdit("1000000")
        self.tbnbinsize.setValidator(qg.QIntValidator())
        labelpromotor=qw.QLabel("Promotor Range")
        ...
        layout.addWidget(self.tbn4,3,1)
        layout.addWidget(label8,4,0)
        layout.addWidget(self.tbn5,4,1)
        layout.addWidget(label9,5,0)
        layout.addWidget(self.tbn6,5,1)
        layout.addWidget(labelpcacut,6,0)
        layout.addWidget(self.tbnpcacut,6,1)
        ...
        #3
        groupBox3 = qw.QGroupBox("Choosing Options")
        ...
        #4
        groupBox4 = qw.QGroupBox("Output Selection")
...
class RunThread(qc.QThread):
    ...
    def run(self):
        ...
        if(w.spfpath=='' or w.igfpath==''):
            ...
            pca_heat_cut=float(w.tbn4.text()) #heatmap least valid max-min to display
            cutoff_CG=float(w.tbn4.text()) #DMR_CG cutoff value
            cutoff_CHH=float(w.tbn5.text()) #DMR_CHH cutoff value
            cutoff_CHG=float(w.tbn6.text()) #DMR_CHG cutoff value
            ...
            self.count.emit(48)
            CG_DMRpd[CG_DMRpd['delta']>0].to_csv(path+'/DMR_CG_hyper.txt',sep='\t',index=None)
            CG_DMRpd[CG_DMRpd['delta']<0].to_csv(path+'/DMR_CG_hypo.txt',sep='\t',index=None)
            CG_DMRpd.to_csv(path+'/DMR_CG_all.txt',sep='\t',index=None)

            CHH_DMRpd[CHH_DMRpd['delta']>0].to_csv(path+'/DMR_CHH_hyper.txt',sep='\t',index=None)
            CHH_DMRpd[CHH_DMRpd['delta']<0].to_csv(path+'/DMR_CHH_hypo.txt',sep='\t',index=None)
            CHH_DMRpd.to_csv(path+'/DMR_CHH_all.txt',sep='\t',index=None)

            CHG_DMRpd[CHG_DMRpd['delta']>0].to_csv(path+'/DMR_CHG_hyper.txt',sep='\t',index=None)
            CHG_DMRpd[CHG_DMRpd['delta']<0].to_csv(path+'/DMR_CHG_hypo.txt',sep='\t',index=None)
            CHG_DMRpd.to_csv(path+'/DMR_CHG_all.txt',sep='\t',index=None)
